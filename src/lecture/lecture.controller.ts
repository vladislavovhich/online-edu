...
export class LectureController {
    ...
    public async findOne(request: Request) {
        ...
        return new Response().json(HttpStatus.OK, { lecture: lectureDto });
    }
    ...
    public async getCourseLectures(request: Request) {
        ...
        return new Response().json(HttpStatus.OK, { lectures });
    }

    public async create(request: Request) {
        const createLectureDto = plainToInstance(
            CreateLectureDto,
            request.body
        );
    ...
    public async update(request: Request) {
        const updateLectureDto = plainToInstance(
            UpdateLectureDto,
            request.body
        );
        const lectureId = parseInt(request.params.id);
        const lecture = await this.lectureService.update(
            lectureId,
            updateLectureDto
        );
        const lectureDto = new GetLectureDto(lecture);
    ...
    public async delete(request: Request) {
        ...
        return new Response().text(
            HttpStatus.OK,
            "Lecture is successfully deleted..."
        );
    }
}
